...
{
	public class MultiFileType : BaseFieldControl
	{
		...
		public override string FieldValue
		{
			get
			{
				...
				string formString = WebUtils.GetFormString("imgselect");
				string formString2 = WebUtils.GetFormString("imgdesc");
				...
				if (array.Length > 0)
				{
					for (int i = 0; i < array.Length; i++)
					{
						...
						if (!string.IsNullOrEmpty(text3))
						{
							...
							if (attachmentInfo.IsPicture)
							{
								attachmentInfo.ImgThumb = text3.Replace(System.IO.Path.GetExtension(text3), "_thumb" + System.IO.Path.GetExtension(text3));
							}
							text = text + Attachment.Add(attachmentInfo) + ",";
						}
					}
				}
				if (text.EndsWith(","))

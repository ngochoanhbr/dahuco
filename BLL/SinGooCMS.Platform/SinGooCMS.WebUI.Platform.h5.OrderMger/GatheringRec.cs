...
{
	public class GatheringRec : H5ManagerPageBase
	{
		...
		private void BindData()
		{
			...
			this.Repeater1.DataSource = SinGooCMS.BLL.GatheringRec.GetPagerList(this.GetCondition(), strSort, this.SinGooPager1.PageIndex, this.SinGooPager1.PageSize, ref recordCount, ref num);
		...
		protected void lnk_Delete_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				GatheringRecInfo dataById = SinGooCMS.BLL.GatheringRec.GetDataById(@int);
				if (dataById == null)
				{
					base.ShowAjaxMsg(this.UpdatePanel1, "Những thông tin này không được tìm thấy, các dữ liệu không tồn tại hoặc đã bị xóa");
				}
				else if (SinGooCMS.BLL.GatheringRec.Delete(@int))
				{
					this.BindData();
					PageBase.log.AddEvent(base.LoginAccount.AccountName, "删除订单[" + dataById.OrderNo + "]的收款单成功");
		...
		protected void btn_DelBat_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				{
					if (SinGooCMS.BLL.GatheringRec.Delete(repeaterCheckIDs))
					{
						this.BindData();
						PageBase.log.AddEvent(base.LoginAccount.AccountName, "批量删除收款单[ID：" + repeaterCheckIDs + "] thành công");
						base.ShowAjaxMsg(this.UpdatePanel1, "Thao tác thành công");
					}
					else
					{

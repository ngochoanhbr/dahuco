...
{
	public class SearchRank : H5ManagerPageBase
	{
		...
		private void BindData()
		{
			int recordCount = 0;
			int num = 0;
			string strSort = " IsRecommend DESC,Times DESC,AutoID DESC ";
			this.SinGooPager1.PageSize = WebUtils.GetInt(this.drpPageSize.SelectedValue);
			this.Repeater1.DataSource = SinGooCMS.BLL.SearchRank.GetPagerList(this.GetCondition(), strSort, this.SinGooPager1.PageIndex, this.SinGooPager1.PageSize, ref recordCount, ref num);
		...
		private string GetCondition()
		{
			...
			{
				text = text + " AND SearchKey like '%" + WebUtils.GetString(this.search_text.Text) + "%' ";
			}
			return text;
		}
		...
		protected void btn_DelBat_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				{
					if (SinGooCMS.BLL.SearchRank.Delete(repeaterCheckIDs))
					{
						this.BindData();
						PageBase.log.AddEvent(base.LoginAccount.AccountName, "批量删除搜索关键字成功");
						base.ShowAjaxMsg(this.UpdatePanel1, "Thao tác thành công");
					}
					else
					{

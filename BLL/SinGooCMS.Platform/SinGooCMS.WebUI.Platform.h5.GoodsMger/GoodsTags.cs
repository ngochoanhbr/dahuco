...
{
	public class GoodsTags : H5ManagerPageBase
	{
		...
		private void BindData()
		{
			...
			this.Repeater1.DataSource = SinGooCMS.BLL.Tags.GetPagerList(this.GetCondition(), strSort, this.SinGooPager1.PageIndex, this.SinGooPager1.PageSize, ref recordCount, ref num);
		...
		private string GetCondition()
		{
			...
			{
				text = text + " AND TagName like '%" + WebUtils.GetString(this.search_text.Text) + "%' ";
			}
			return text;
		}
		...
		protected void btn_DelBat_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				{
					if (SinGooCMS.BLL.Tags.Delete(repeaterCheckIDs))
					{
						this.BindData();
						PageBase.log.AddEvent(base.LoginAccount.AccountName, "批量删除标签成功");
						base.ShowAjaxMsg(this.UpdatePanel1, "Thao tác thành công");
					}
					else
					{

...
{
	public class IPStrategy : H5ManagerPageBase
	{
		...
		private void BindData()
		{
			...
			this.Repeater1.DataSource = SinGooCMS.BLL.IPStrategy.GetPagerList(this.GetCondition(), strSort, this.SinGooPager1.PageIndex, this.SinGooPager1.PageSize, ref recordCount, ref num);
		...
		private string GetCondition()
		{
			...
			{
				text = text + " AND IPAddress like '%" + WebUtils.GetString(this.search_text.Text) + "%' ";
			}
			return text;
		}
		...
		protected void btn_DelBat_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				if (!string.IsNullOrEmpty(repeaterCheckIDs) && SinGooCMS.BLL.IPStrategy.Delete(repeaterCheckIDs))
				{
					this.BindData();
					PageBase.log.AddEvent(base.LoginAccount.AccountName, "批量删除IP策略成功");
					base.ShowAjaxMsg(this.UpdatePanel1, "Thao tác thành công");
				}
				else
				{

...
{
	public class AccountDetail : H5ManagerPageBase
	{
		...
		private void BindData()
		{
			...
			this.Repeater1.DataSource = SinGooCMS.BLL.AccountDetail.GetPagerList(this.GetCondition(), strSort, this.SinGooPager1.PageIndex, this.SinGooPager1.PageSize, ref recordCount, ref num);
		...
		protected void btn_DelBat_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				if (!string.IsNullOrEmpty(repeaterCheckIDs) && SinGooCMS.BLL.AccountDetail.Delete(repeaterCheckIDs))
				{
					this.BindData();
					PageBase.log.AddEvent(base.LoginAccount.AccountName, "批量删除账户明细成功");
					base.ShowAjaxMsg(this.UpdatePanel1, "Thao tác thành công");
				}
				else
				{

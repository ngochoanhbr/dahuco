...
{
	public class UserLevel : H5ManagerPageBase
	{
		...
		private void BindData()
		{
			int recordCount = 0;
			int num = 0;
			string strSort = " Integral ASC ";
			this.SinGooPager1.PageSize = WebUtils.GetInt(this.drpPageSize.SelectedValue);
			this.Repeater1.DataSource = SinGooCMS.BLL.UserLevel.GetPagerList(this.GetCondition(), strSort, this.SinGooPager1.PageIndex, this.SinGooPager1.PageSize, ref recordCount, ref num);
		...
		private string GetCondition()
		{
			...
			{
				text = text + " AND UserLevel like '%" + WebUtils.GetString(this.search_text.Text) + "%' ";
			}
			return text;
		}

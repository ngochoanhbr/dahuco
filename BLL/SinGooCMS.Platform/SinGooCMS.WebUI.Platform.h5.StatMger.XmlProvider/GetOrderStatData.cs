...
{
	public class GetOrderStatData : IHttpHandler
	{
		...
		public void ProcessRequest(HttpContext context)
		{
			...
			{
				DataTable orderStat = Stat.GetOrderStat(queryString2, dateTime, dateTime2);
				...
				if (queryString2.Equals("年"))
				{
					...
					stringBuilder.Append("</categories>");
					stringBuilder.Append("<dataset seriesName='订单数量' color='1D8BD1' anchorBorderColor='1D8BD1' anchorBgColor='1D8BD1'>");
					for (int i = dateTime.Year; i <= dateTime2.Year; i++)
					{
						DataRow[] array = orderStat.Select("Title='" + i.ToString() + "'");
						string text3 = (array != null && array.Length > 0) ? array[0]["OrderNum"].ToString() : "0";
						stringBuilder.Append(string.Concat(new string[]
						{
							"<set value='",
							text3,
				...
				else if (queryString2.Equals("周"))
				{
					...
					while (t <= System.DateTime.Now)
					{
						DataRow[] array = orderStat.Select("Title='" + t.ToString("MM-dd") + "'");
						string text3 = (array != null && array.Length > 0) ? array[0]["OrderNum"].ToString() : "0";
						stringBuilder.Append(string.Concat(new string[]
						{
							"<set value='",

<%@ Page Title="" Language="C#" MasterPageFile="~/Platform/h5/MasterPage/MasterDefault.Master"
    AutoEventWireup="true" CodeBehind="Coupons.aspx.cs" Inherits="SinGooCMS.WebUI.Platform.h5.MarketMger.Coupons" %>
                    ...
                    <div class="profile-body mb-20">
                        <div class="form-group fix hidden">
                            <label for="firstname" class="sr-only">
                                singoocms</label>
                            <div class="col-md-3">
                                <asp:TextBox ID="search_text" runat="server" CssClass="form-control" placeholder="Xin vui lòng nhập mã phiếu giảm giá"></asp:TextBox>
                            </div>
                            <label for="firstname" class="sr-only">
                                singoocms</label>
                            <div class="col-md-3">
                        ...
                        <div class="batchHandleArea fix">
                            ...
                            <div class="btn-group">
                                <button type="button" class="btn btn-warning" onclick="location='ModifyCoupons.aspx?CatalogID=<%=base.CurrentCatalogID %>&Module=<%=base.CurrentModuleCode %>&action=Add'">
                                    Thêm</button>
                            </div>
                            <div class="fr">
                                <div class="col-md-12">
                                ...
                                <asp:Repeater ID="Repeater1" runat="server">
                                    <ItemTemplate>
                                        <tr>
                                            ...
                                            <td>
                                                <%#Convert.ToDecimal(Eval("Notes")).ToString("f2")%>đ
                                                (Chi phí đơn lẻ <%#Convert.ToDecimal(Eval("Touch")).ToString("f2")%> đ)
                                            </td>
                                            <td>
                                                <%#Convert.ToDateTime(Eval("StartTime")).ToString("yyyy-MM-dd")%>
                                                -
                                                <%#Convert.ToDateTime(Eval("EndTime")).ToString("yyyy-MM-dd")%>
                                                <%#(System.DateTime.Now-Convert.ToDateTime(Eval("EndTime"))).TotalDays>1 ? "<span style='color:red'>Hết hạn</span>" : ""%>
                                            </td>
                                            <td>
                                                <%#Eval("IsUsed").ToString() == "True" ? "<i class=\"iconfont font-22 text-success\">&#xe62f;</i>" : "<i class=\"iconfont font-22 \">&#xe62e;</i>"%>
                                            </td>
                                            <td class="text-muted">
                                                <a href="javascript:void(0)" onclick="location='ModifyCoupons.aspx?CatalogID=<%=base.CurrentCatalogID %>&Module=<%=base.CurrentModuleCode %>&action=Modify&opid=<%#Eval("AutoID") %>'">Sửa</a>
                                                <a href="javascript:void(0)" onclick="$.dialog.open('CopyCoupons.aspx?CatalogID=<%=base.CurrentCatalogID %>&Module=<%=base.CurrentModuleCode %>&action=Copy&opid=<%#Eval("AutoID") %>',{title:'Copy phiếu giảm giá',width:500,height:150},false);">Copy</a>
